package br.com.kalendae.test.controller;

import java.util.List;

import org.springframework.web.bind.annotation.DeleteMapping;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.PutMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

import br.com.kalendae.test.dto.PessoaDto;
import br.com.kalendae.test.exception.RegistroNaoEncontradoException;
import br.com.kalendae.test.service.PessoaService;
import lombok.AllArgsConstructor;

@RestController
@RequestMapping("/pessoas")
@AllArgsConstructor
public class PessoaController {

	private final PessoaService pessoaService;

	@GetMapping
	public List<PessoaDto> listarTodos() {
		return pessoaService.listarTodos();
	}

	@GetMapping("/{id}")
	public PessoaDto recuperarPorId(@PathVariable Long id) throws RegistroNaoEncontradoException {
		return pessoaService.recuperarPorId(id);
	}
	
	@PostMapping
	public PessoaDto inserir(@RequestBody PessoaDto pessoaDto) {
		return pessoaService.inserir(pessoaDto);
	}

	@PutMapping
	public void atualizar(@RequestBody PessoaDto pessoaDto) throws RegistroNaoEncontradoException {
		pessoaService.atualizar(pessoaDto);
	}

	@DeleteMapping("/{id}")
	public void remover(@PathVariable Long id) throws RegistroNaoEncontradoException {
		pessoaService.remover(id);
	}

}
